...
## `webshot` {#pdf-realmeter-webshot}

`webshot` PDF 파일 찍는 것은 잘 동작하지 않네요!!!

```{r realmeter-webshot, eval = FALSE}
webshot::webshot("http://www.realmeter.net/wp-content/uploads/2017/04/realmeter_w17041321.pdf", "fig/realmeter-pdf.png", delay = 3)
```

## 표 추출 {#pdf-realmeter-webshot-tabulizer}


```{r extract-table-from-pdf, eval = FALSE}
library(tabulizer)

## 여론조사 다운로드
download.file(url="http://www.realmeter.net/wp-content/uploads/2017/04/realmeter_w17041321.pdf", destfile = "data/realmeter_w17041321.pdf")

realmeter_table <- extract_tables("data/realmeter_w17041321.pdf",
  pages=6,
  output = "data.frame",
  method = "decide",
  # guess = TRUE,
  area = list(c(75, 472.62, 75+447.83, 472.62+292.47)))

realmeter_df <- realmeter_table %>% .[[1]] %>% janitor::clean_names()
